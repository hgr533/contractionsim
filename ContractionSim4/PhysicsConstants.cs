...
internal class PhysicsConstants
{
    ...
    //public double TunnelingProbability(double barrier) => Math.Exp(-barrier);

    //// Relativistic corrections for high-energy states
    //public double LorentzFactor(double velocity) => 1 / Math.Sqrt(1 - velocity * velocity / (LightSpeed * LightSpeed));
}